<?php
	...
	function cash_form(){
		...
		$this->Sales_model->insert_bill($bill_no,$custname,$cust_id,$form_data["veh_no"],$form_data["mob_no"],$form_data["pump_no"],$form_data["shift"],$form_data["counterno"],$form_data["sales_mode"],$form_data["total"],$indent_no,$form_data["km_rdng"],$form_data["acct_date"],$twotpumpno,$cheque_no,$cheque_date,$cheque_status,$bank_name,$ref_no);
		$cnt=$form_data["count"];
		for ($i = 1; $i <= $cnt; $i++) {
		...
		$this->Sales_model->update_rfid_entry($bill_no,$form_data["veh_no"]);
		 if(!($form_data["sales_mode"]=='Indent_sales'))
		{
		if (preg_match("/[^a-zA-Z'-]/",$form_data["veh_no"])) {
		$vehicle=$form_data["veh_no"];	
		if(!is_numeric($vehicle)){	
			$rowcount=$form_data["rowcount"];
			if($rowcount == 0)
			{
			 $this->Sales_model->insert_retailcustomer($form_data["veh_no"],$form_data["cust_name"],$form_data["mob_no"]);
			}else{
			 $this->Sales_model->update_retailcustomer($form_data["veh_no"],$form_data["cust_name"],$form_data["mob_no"]);
			}
		}
		}
		}
	...
	function other_sales_form(){
		...
		$this->Sales_model->insert_other_pdt_bill($bill_no,$custname,$cust_id,$form_data["veh_no"],$form_data["mob_no"],$form_data["shift"],$form_data["counterno"],$form_data["sales_mode"],$form_data["total"],$form_data["acct_date"],$indent_no,$cheque_no,$cheque_date,$cheque_status,$bank_name,$ref_no);
		...
		if($form_data["sales_mode"]=='Indent_sales'){
			...
			if($rowcount == 0)
			{
		 	$this->Sales_model->insert_other_pdts_customer($form_data["veh_no"],$form_data["cust_name"],$form_data["mob_no"],$form_data["current_custid"],$form_data["next_custid"]);
		 	
			}else{
				
			 $this->Sales_model->update_other_pdts_customer($form_data["veh_no"],$form_data["cust_name"],$form_data["mob_no"],$form_data["cust_id"]);
			}
			}
			echo $bill_no;
	...
	function retailbill_update(){
		...
		$this->Sales_model->update_retailbill_info($collect["bill_no"],$collect["cust_name"],$collect["veh_no"],$collect["mob_no"],$collect["pump_no"],$collect["shift"],$collect["counter"],$collect["sales_mode"],$collect["total"],$collect["indent_no"],$collect["meter_reading"],$collect["cust_id"],$collect["twotoilpump"],$collect["cheque_no"],$collect["cheque_date"],$collect["cheque_status"],$collect["bank_name"],$collect["ref_no"]);
		echo "Bill Information Update Successfully";
	}	
	...
	function update_other_product_bill(){
		...
		$this->Sales_model->update_other_product_bill($collect["bill_no"],$collect["cust_name"],$collect["veh_no"],$collect["mob_no"],$collect["shift"],$collect["counter"],$collect["sales_mode"],$collect["total"],$collect["indent_no"],$collect["cust_id"],$collect["cheque_no"],$collect["cheque_date"],$collect["cheque_status"],$collect["bank_name"],$collect["ref_no"]);	
		echo "Bill Information Update Successfully";
	}
