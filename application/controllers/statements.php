...
function index()
	...
	function cumulative_sales_stmt_rpt(){
		...
		$data['start_date']=$form_data['sdate'];
		$data['end_date']=$form_data['edate'];
		$data["sales_stmt"]=$this->Statements_model->get_cumulative_sales_stmt($form_data["sdate"],$form_data["edate"]);
		...
		$start=$form_data[0];
		$end=$form_data[1];
		$stmt=$this->Statements_model->get_cumulative_sales_stmt($start,$end);
