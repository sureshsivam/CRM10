...
class Reports_model extends CI_Model{
	...
	function other_pur_report($sdate,$edate,$supp){
		if($supp=='default'){
			...
			$whr="and party_name='$supp'";
		}
		return $this->db->query("select * from other_pur_entry where account_date between '$sdate' and '$edate' $whr ")->result_array();
	}
	...
	function ind_sal_report_details($bill_no){
		...
		(select bill_no,product,quantity,rate,value,product_category,added_by,bill_time from bill_details
		union
		select bill_no,product,quantity,rate,value,category,added_by,bill_time from other_pdts_bill_details) as bill_table  
		where bill_no='$bill_no'")->result_array();
	}
	...
	function get_pet_bills($sdate,$edate){
		return $this->db->query("select * from retail_bills where acct_date between '$sdate' and '$edate'")->result_array();
	}
	...
	function get_other_bills($sdate,$edate){
		return $this->db->query("select * from other_pdts_bill where acct_date between '$sdate' and '$edate'")->result_array();
	}
	...
	function ind_stmt_payment_report($sdate,$edate,$cust_name,$payment_mode){
		...
		return $this->db->query("Select c.customer_name,c.tin,a.bill_no,b.payment_mode,b.amount,b.payment_date,b.cheque_no,b.cheque_date,b.bank_name,b.clearance_date from indent_statement_bills a join indent_stmt_payments b on b.bill_no=a.bill_no join customer_master c on c.customer_id=a.cust_id where a.bill_date between '$sdate' and '$edate' $paymode $custid")->result_array();
	}
	...
	if($cust_name=='all'){
		...
		return $this->db->query("Select a.*,ifnull((select sum(amount) from indent_stmt_payments c where c.bill_no=a.bill_no),0) as amount,b.customer_name,b.tin from indent_statement_bills a join customer_master b on b.customer_id=a.cust_id where bill_date between '$sdate' and '$edate' $whr")->result_array();
	}

	function cheque_sal_report($sdate,$edate){
		return $this->db->query("Select bill_number,acct_date ,bank_name,customer_name,vehicle_number,total_amount,cheque_date,cheque_no,cheque_status
			from retail_bills where cheque_no!='NULL'and acct_date between '$sdate' and '$edate'")->result_array();
	}
