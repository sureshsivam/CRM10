...
class Sales_model extends CI_Model
{
	...
	function get_retail_trucks(){
		$result=$this->db->query("select * from retail_customers")->result_array();
		if(!empty($result)){
		foreach($result as $res){
...
function get_pump_of_counter_sales_entry($counter)
	...
	{	
		...
		$this->db->query("insert into retail_customers values('','$customer_name','$vehicle_number','$mobile_number','$cust_id','$uname','$add_date')");
		$new_cust_id=++$cust_id;
	...
	function insert_other_pdts_customer($vehicle_number,$customer_name,$mobile_number,$current_custid,$next_custid)
	{
		$uname=$this->session->userdata('admin_user_email');
		$add_date=date('Y-m-d H:i:s');
		$this->db->query("insert into retail_customers values('','$customer_name','$vehicle_number','$mobile_number','$current_custid','$uname','$add_date')");
		$this->db->query("UPDATE bill_no_generator SET other_pdts_cust_id='$next_custid'");
	}
	function update_other_pdts_customer($vehicle_number,$customer_name,$mobile_number,$current_custid)
	{
		$uname=$this->session->userdata('admin_user_email');
		$add_date=date('Y-m-d H:i:s');
		return $this->db->query("UPDATE retail_customers 
		SET customer_name='$customer_name' ,mobile_number='$mobile_number',added_date='$add_date', user='$uname',vehicle_number='$vehicle_number'
	...
	function get_2toilpump_list(){
		return $this->db->query("select pump_no from pump_master where status='1' and product_name IN ('2TOIL_LOOSE')")->result_array();
	}
	...
	function insert_sale_entry($form_data){
		...
		{
			$this->db->query("update petrol_sales_entry set open_reading='".$form_data["open_rdng"]."',close_reading='".$form_data["close_rdng"]."',sales_litres='".$form_data["sales_ltrs"]."',test_litres='".$form_data["test_ltrs"]."',net_sales='".$form_data["net_sales"]."',rate='".$form_data["rate"]."',amount='".$form_data["amt"]."',added_date='".$add_date."',added_by='".$uname."' where acct_date='".$form_data["acct_date"]."' and shift='".$form_data["shift"]."' and pump_no='".$form_data["pump_no"]."'" );
		}
		$this->db->query("update pump_master set last_close_reading='".$form_data["close_rdng"]."' where pump_no='".$form_data["pump_no"]."'");
	}
	
	function insert_other_pdt_bill($bill_no,$cust_name,$cust_id,$veh_no,$mob_no,$shift,$counterno,$sales_mode,$total,$acct_date,$indent_no,$cheque_no,$cheque_date,$cheque_status,$bank_name,$ref_no){
		$uname=$this->session->userdata('admin_user_email');
		$add_date=date('Y-m-d H:i:s');
...
function get_product_info()
	...
	function update_retailbill_info($bill_no,$cust_name,$veh_no,$mob_no,$pump_no,$shift,$counter,$sales_mode,$total,$indent_no,$meter_reading,$cust_id,$twotoilpump,$cheque_no,$cheque_date,$cheque_status,$bank_name,$ref_no)
	{
	$uname=$this->session->userdata('admin_user_email');
	$add_date=date('Y-m-d H:i:s');
	...
	{
		$query=$this->db->query("SELECT product_name FROM product_master WHERE category IN ('oil','Grease','others');");
		return $query->result();
	}
	...
	{
	$cust_id=$this->db->query("SELECT customer_id FROM crm.customer_master
	...
	function get_indent_limit($cust_id){
		$cust_id=$this->db->query("SELECT indent_limit,indent_taken FROM crm.customer_master
											where customer_id='$cust_id'")->result();
		foreach($cust_id as $cust){
		$indent_limit=$cust->indent_limit;
		$indent_taken=$cust->indent_taken;
	
	}
		return $indent_limit."::".$indent_taken;
	}
...
function update_other_product_bill($bill_no,$cust_name,$veh_no,$mob_no,$shift,$counter,$sales_mode,$total,$indent_no,$cust_id,$cheque_no,$cheque_date,$cheque_status,$bank_name,$ref_no)
	{
	$uname=$this->session->userdata('admin_user_email');
	$add_date=date('Y-m-d H:i:s');
...
function check_indent_no($cust_id)
	{
	$Indent_no=$this->db->query("SELECT indent_start_no,indent_end_no FROM crm.customer_master
	...
	{
		$retail_cust=$this->db->query("SELECT vehicle_number,mobile_number FROM crm.retail_customers
		...
		return $vehicle_number."!".$mobile_number."!".$rowcount;
	...
	function update_cancelled_testing_bills($bill_no,$reason)
		...
		$query=$this->db->query("SELECT *  
								 ...
								 where DATE(cancelled_time) BETWEEN '$start' and '$end'
								 ");
		return $query->result();
		}
