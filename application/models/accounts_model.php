...
function get_bill_details($bill_no){
	return $this->db->query("SELECT *,(SELECT ifnull(SUM(amount),0) FROM indent_stmt_payments WHERE bill_no='$bill_no') AS paid_amount FROM indent_statement_bills WHERE bill_no='$bill_no'")->result_array();
}
...
(SELECT (SELECT opening_balance FROM customer_master  WHERE customer_id='$cust_id') AS OPEN_BAL,(SELECT SUM(bill_amount) FROM indent_statement_bills  WHERE cust_id='$cust_id') AS BILL, (SELECT SUM(b.amount) FROM indent_statement_bills a join indent_stmt_payments b on b.bill_no=a.bill_no WHERE a.cust_id='$cust_id') AS PAID) AS C")->result_array();
}
