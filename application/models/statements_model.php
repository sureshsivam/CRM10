...
SELECT total_amt as total from other_pdts_bill where indent_stmt_no='$bill_no' ) as tot_table) where bill_no='$bill_no'
	...
	function indent_stmt_details($sdate,$edate){
		return $this->db->query("Select a.*,b.customer_name from indent_statement_bills a join customer_master b on b.customer_id=a.cust_id where bill_date between '$sdate' and '$edate'")->result();
	}
	...
	function update_indentstmtno($bill_no,$sdate,$edate,$cust_id){
		$this->db->query("Update retail_bills set indent_stmt_no='IS$bill_no' where sale_mode='Indent_sales'and acct_date between '$sdate' and '$edate' and cust_id='$cust_id' ");
		$this->db->query("Update other_pdts_bill set indent_stmt_no='IS$bill_no' where sale_mode='Indent_sales'and acct_date between '$sdate' and '$edate' and cust_id='$cust_id' ");
	}
	...
	function get_cumulative_sales_stmt($start_date,$end_date){
...
from
(SELECT c.category,b.acct_date FROM petrol_sales_entry b join pump_master a on a.pump_no=b.pump_no join product_master c on c.product_name=a.product_name where b.acct_date between '$start_date' and '$end_date' group by b.acct_date,c.category) as product  
left join 
...
select product.acct_date,'OTHERS',indent.sales as 'Indent_sales',credit.sales as 'Credit_card_sales',xreward.sales as 'XtraReward_Sales',xpower.sales as 'XtraPower_sales',easy.sales as 'Easy_fuel_sales',cheque.sales as 'Cheque_sales', ifnull(tot.sales,0) - (ifnull(indent.sales,0) + ifnull(credit.sales,0) + ifnull(xreward.sales,0) + ifnull(xpower.sales,0) + ifnull(easy.sales,0) + ifnull(cheque.sales,0)) as 'Cash_sales',tot.sales as 'Total_sales'
from
(SELECT b.category,a.acct_date FROM other_pdts_bill_details b join other_pdts_bill a on a.bill_no=b.bill_no where a.acct_date between '$start_date' and '$end_date' group by a.acct_date ) as product  
left join 
...
) as cheque on  cheque.acct_date=product.acct_date
