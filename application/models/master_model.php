...
function check_product($prod_name){
	...
	function insert_customerdtls($cust_name,$addr,$ph_num,$tin,$vehicle,$indent_no_start,$indent_no_end,$current_custid,$next_custid,$initial_deposit,$indent_limit,$open_bal)
	{
		$uname=$this->session->userdata('admin_user_email');
		$mydate=date('Y-m-d H:i:s');
		$cust=mysql_real_escape_string($cust_name);
		$this->db->query("INSERT INTO customer_master(customer_name,address,phone_number,tin,vehicle_number,indent_start_no,indent_end_no,customer_id,updated_by,updated_date,initial_deposit,indent_limit,opening_balance) VALUES(\"$cust\",'$addr','$ph_num','$tin','$vehicle','$indent_no_start','$indent_no_end','$current_custid','$uname','$mydate','$initial_deposit','$indent_limit','$open_bal')");
...
function update_customer_info($cust_name,$cust_id,$addr,$phone_number,$tin,$vehicle_number,$indent_start_no,$indent_end_no,$initial_deposit,$indent_limit,$open_bal)
	{
		
		$uname=$this->session->userdata('admin_user_email');
		$mydate=date('Y-m-d H:i:s');
		$this->db->query("update customer_master set customer_name='$cust_name',address='$addr',phone_number='$phone_number',tin='$tin',vehicle_number='$vehicle_number',indent_start_no='$indent_start_no',indent_end_no='$indent_end_no',updated_by='$uname',updated_date='$mydate',initial_deposit='$initial_deposit',indent_limit='$indent_limit',opening_balance='$open_bal' WHERE customer_id='$cust_id'");
	}
